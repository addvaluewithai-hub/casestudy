...
- [ ] Run PageSpeed on final portfolio case-study page. Run 43 produced the first retained deployed-preview baseline (mobile Performance 80 / LCP 4.78s; desktop Performance 95 / LCP 1.32s) and exposed an ~8.9 MB payload dominated by original PNG evidence. The optimized portfolio commit is now confirmed deployed. Audit commit `b2e92aa237daba8a5e6f038cf5cf081fc25b0800` records total byte weight/top resources and fails if any known Alamaar evidence image is still requested without `/image/upload/f_auto/q_auto/`; close this item only after the fresh triggered run proves transformed delivery and records the post-optimization scores/payload.
