...
- [ ] Add responsive/mobile QA for the case-study page. Run 43 is fully green at 1440/834/390px with HTTP 200, current H1, all required narrative headings, no runtime errors and no horizontal overflow. Retained full-page screenshots are also copied to Cloudinary as `qa-run43-desktop`, `qa-run43-tablet`, and `qa-run43-mobile`; visual review still required before closing this item. The QA now also forces lazy evidence images into view before screenshots and verifies all expected deployed image URLs use `f_auto/q_auto`.
...
- [x] Optimize Alamaar evidence image delivery on the portfolio page. Commit `19637914b7ff6320aa443a89847387bd6b22eb36` routes Cloudinary image evidence through `f_auto/q_auto` after run 43 showed the original PNGs dominated an ~8.9 MB page payload; Cloudflare Pages has confirmed this commit deployed successfully.
...
- [ ] Test case study at desktop/tablet/mobile. Run 43 automated checks are green at 1440, 834 and 390px with no overflow/runtime failures; visually review retained screenshots before closing.
- [ ] Test video autoplay/mute/playsInline/reduced-motion behavior. Run 43 reports normal muted/playsInline/metadata behavior and reduced-motion autoplay=false, loop=false, controls=true, paused=true. Keep grouped with the final visual review before closing.
- [ ] Run PageSpeed on final portfolio case-study page. Run 43 produced the first retained deployed-preview baseline (mobile Performance 80 / LCP 4.78s; desktop Performance 95 / LCP 1.32s) and exposed an ~8.9 MB payload dominated by original PNG evidence. The optimized portfolio commit is now confirmed deployed. Lighthouse audit commit `b2e92aa237daba8a5e6f038cf5cf081fc25b0800` records total byte weight/top resources and flags unoptimized known Alamaar evidence requests; Playwright QA commit `28000fff5efb5dd902ffbf3410b0a9e10a801794` independently verifies all 12 expected evidence images in the deployed DOM use `/image/upload/f_auto/q_auto/`, covering below-fold lazy images that Lighthouse may not request. Close only after the fresh triggered run records post-optimization scores/payload and both delivery checks pass.
