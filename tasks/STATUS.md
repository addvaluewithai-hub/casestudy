...
- Diagnosed a second QA evidence blind spot: the `casestudy` cross-repository workflow was updated to the correct release-preview ref, but run 4 still published `build/crawlable/visual: skipped`, so it is not valid responsive evidence. Rather than infer a pass, the portfolio branch's own `Alamaar rebuild release QA` now has `contents: write` and publishes `qa-artifacts/alamaar/{desktop,tablet,mobile}.png`, `report.json`, and `run-status.json` back onto `feat/alamaar-rebuild-release-preview` after each run. This makes the evidence inspectable through the normal GitHub file connector without relying on transient Actions artifacts or cross-repository checkout. Publishing change: `5c75b00df421762ad09729ec0f10bfb3be395ae3`.

## Immediate next work

1. Inspect `qa-artifacts/alamaar/run-status.json`, `report.json`, and desktop/tablet/mobile screenshots once the self-publishing release QA lands; fix any build, overflow, runtime or reduced-motion issue before closing responsive QA.
